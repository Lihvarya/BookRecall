...
- 查看本轮工具 trace、工具调用路径、命中数和防剧透触发次数
...
- 更完整的会话分支分析，例如分支合并、更深层差异摘要和产品级并排对比视图
- 更完整的 trace 可视化，例如真实耗时采集、更强图形化路径和跨分支 trace 差异分析
...
- 已支持会话 ID、会话/分支列表、基础分支差异对比、长会话历史、本轮工具轨迹汇总、工具路径展示、历史轮次 trace 回放、历史轮次编辑/删除/重新提问，以及从指定轮次删除后续历史并重算或新建分支
...
- 产品级前端工程能力，例如组件化、路由、可视化关系图谱、分支合并和更完整的错误态设计
