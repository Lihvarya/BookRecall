...
## 2. 还没有原生 function-calling

现状：

- `LLMReActPolicy` 走的是文本协议解析
- 不是原生 OpenAI tool calling

结果是：

- 对模型输出格式更敏感
- 对不同 OpenAI-compatible 供应商兼容性一般
- 有回退，但不够优雅

这也是当前 Agent 层最值得优先升级的一项。
...
2. 跨会话 Agent 记忆
3. FAISS / 更强的 embedding 检索后端
4. 人物关系与主题线索层
5. Web 端多轮对话与 trace 可视化
6. LangGraph 真落地

原因很简单：

- `function-calling` 能直接提升云端 Agent 稳定性
...
- 把 `LLMReActPolicy` 升级为原生 tool calling 优先，文本协议作为回退
